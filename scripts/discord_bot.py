...
"""Managed Discord soccer-bot process for the eeesoc EC2 host.

Wraps ``eeesoc.discorder`` with a PID / --status / --stop / --replace
lifecycle so ``./restart-discord-bot.sh`` (and ``deploy.sh``) can start
it detached inside the dashboard container.
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(
        description="eeesoc Discord soccer bot (PID-managed)",
    ...
    try:
        ...
        print(f"[{datetime.now():%H:%M:%S}] starting discord soccer bot…")
        run_bot()
    finally:
        _clear_pid(pid_file)
    return 0
