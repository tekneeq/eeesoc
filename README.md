...
an xG feed: shots 35%, SOT 25%, corners 15%, goals 25%). 50 = a league-typical attack, above 50 is
...
par), so 50 = allows the league's typical chances, above 50 is *solid*, below *leaky*, ranked the same way
...
- **9:30 AM ET** every day — today's scheduled matches in MLS, Premier League, Championship, Carabao Cup, Ligue 1, La Liga, Bundesliga, Serie A, Champions League, Europa League, and Conference League (kickoff times in ET). If the bot starts later the same morning, it still posts once. `!soc today` reprints that slate.
